...
void append(vector<node_value_type> & alphalist, node_value_type alpha_, Manner manner_) {
  if (alpha_ > th) {
...
void append(vector<node_value_type> & alphalist, node_value_type alpha_, Manner manner_, int split_) {
  if (alpha_ > th) {
...
void append(vector <node_value_type> & alphalist, node_value_type alpha_, Manner manner_, char l1_, int l2_) {
  if (alpha_ > th) {
    BackPointer backpointer;
...
private:
  ...
  node_value_type **bestH, **bestP, **bestM2, **bestMulti, **bestM;
  // map<int, node_value_type *> *sortedP;
  vector<int> *sortedP;
  // State *bestC;
  node_value_type *bestC;
  ...
  node_value_type & get_state(int i, int j, Type type);
  node_value_type ** get_states(Type type);
